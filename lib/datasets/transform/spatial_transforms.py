...
def augment_channel_translation(data_list, const_channel=0, max_shifts=None):
    ...
    if const_channel is not None:
        ...
        trans_data_list = [data[:, [i for i in range(shape[1]) if i != const_channel]] for data in data_list]
    else:
    ...
    for j in range(shape[0]):
        ...
        if len(shape) == 5:
            ...
            slices = [slice[:, ixs['z']['lo']:ixs['z']['hi'], ixs['y']['lo']:ixs['y']['hi'],
                    ixs['x']['lo']:ixs['x']['hi']] for slice in slices]
            slices = [np.pad(slice, ((0, 0), (pad['z']['lo'], pad['z']['hi']), (pad['y']['lo'], pad['y']['hi']),
                                   (pad['x']['lo'], pad['x']['hi'])),
                           mode='constant', constant_values=(0, 0)) for slice in slices]
        if len(shape) == 4:
            # slice = slice[:, ixs['y']['lo']:ixs['y']['hi'], ixs['x']['lo']:ixs['x']['hi']]
            # slice = np.pad(slice, ((0, 0), (pad['y']['lo'], pad['y']['hi']), (pad['x']['lo'], pad['x']['hi'])),
            #                mode='constant', constant_values=(0, 0))

            slices = [slice[:, ixs['y']['lo']:ixs['y']['hi'], ixs['x']['lo']:ixs['x']['hi']] for slice in slices]
            slices = [np.pad(slice, ((0, 0), (pad['y']['lo'], pad['y']['hi']), (pad['x']['lo'], pad['x']['hi'])),
                           mode='constant', constant_values=(0, 0)) for slice in slices]
...
def crop(data, seg=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    if labels_extra is not None:
        ...
        for ith in range(len(labels_extra)):
            seg_extra_outputs.append(np.zeros([seg_shape[0], seg_shape[1]] + list(crop_size), dtype=labels_extra[ith].dtype))
    else:
    ...
    for b in range(data_shape[0]):
        ...
        if any([i > 0 for j in need_to_pad for i in j]):
            ...
            if labels_extra is not None:
                for ith in range(len(labels_extra)):
                    seg_extra_outputs[ith][b] = np.pad(seg_extra_cropped[ith], need_to_pad, pad_mode_seg, **pad_kwargs_seg)
        else:
...
def random_crop_aug(data, seg=None, crop_size=128, margins=[0, 0, 0], labels_extra=None):
    return crop(data, seg, crop_size, margins, 'random', labels_extra=labels_extra)


def augment_spatial(data, seg, patch_size, labels_extra=None, patch_center_dist_from_border=30,
    ...
    if labels_extra is not None:
        for _ in range(len(labels_extra)):
            if dim == 2:
                seg_extra_outputs.append(np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1]), dtype=np.float32))
            else:
                seg_extra_outputs.append(np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                      dtype=np.float32))
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            if labels_extra is not None:
                for ith, seg_arr in enumerate(labels_extra):
                    for channel_id in range(seg_arr.shape[1]):
                        seg_extra_outputs[ith][sample_id, channel_id] = interpolate_img(seg_arr[sample_id, channel_id], coords, order_seg,
                                                                            border_mode_seg, cval=border_cval_seg,
                                                                            is_seg=True)
        else:
            if seg is None:
            ...
            if labels_extra is not None:
                labels_extra_sample = []
                for ith in range(len(labels_extra)):
                    labels_extra_sample.append(labels_extra[ith][sample_id:sample_id + 1])
            else:
                labels_extra_sample = None

            if random_crop:
                margin = [patch_center_dist_from_border[d] - patch_size[d] // 2 for d in range(dim)]
                d, s, seg_extra_output = random_crop_aug(data[sample_id:sample_id + 1], s, patch_size, margin,
                                                          labels_extra=labels_extra_sample)
            else:
                d, s, seg_extra_output = center_crop_aug(data[sample_id:sample_id + 1], patch_size, s,
                                                         labels_extra=labels_extra_sample)
...
class SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size, patch_center_dist_from_border=30,
                 ...
                 do_translate=False, p_trans=1, trans_max_shifts={'z': 2, 'y': 2, 'x': 2}, trans_const_channel=None,
    ...
    def __call__(self, **data_dict):
        ...
        if self.extra_label_keys is not None:
            labels_extra = [data_dict.get(extra_label_key) for extra_label_key in self.extra_label_keys]
        else:
            labels_extra = None
        ...
        if self.do_translate and np.random.uniform()<self.p_trans:
            ...
            trans_list = augment_channel_translation(tensor_list, const_channel=self.trans_const_channel,
...
class MirrorTransform(AbstractTransform):
    ...
    def __init__(self, axes=(0, 1, 2), data_key="data", label_key="seg", extra_label_keys=None):
        self.data_key = data_key
        self.label_key = label_key
        self.axes = axes
        if max(axes) > 2:
    ...
    def __call__(self, **data_dict):
        ...
        for b in range(len(data)):
            ...
            ret_val = augment_mirroring(data[b], sample_seg, axes=self.axes, labels_extra=labels_extra_slices)
            data[b] = ret_val[0]
            if seg is not None:
                seg[b] = ret_val[1]
...
class RemoveLabelTransform(AbstractTransform):
    ...
    def __init__(self, remove_label, replace_with=0, input_key="seg", output_key="seg",
                 extra_label_keys=None, extra_remove_label=-1, extra_replace_with=255):
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        # dataset
        dataloader_train = BGDataset(data_dir, batch_size, phase='train', shuffle=False,
                                     use_weak=True, use_duplicate=True)
        ...
        tr_transforms.append(SpatialTransform(
            ...
            do_translate=False, trans_max_shifts={'z': 2, 'y': 2, 'x': 2}, trans_const_channel=None,
