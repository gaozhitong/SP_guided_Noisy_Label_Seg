...
class DownsampleSegForDSTransform2(AbstractTransform):
    ...
    '''
    def __init__(self, ds_scales=(1, 0.5, 0.25), order=0, cval=0, input_key="seg", output_key="seg", axes=None, extra_label_keys=None):
        self.axes = axes
        self.output_key = output_key
        self.input_key = input_key
        self.cval = cval
    ...
    def __call__(self, **data_dict):
        ...
        if self.extra_label_keys is not None:
            for key in self.extra_label_keys:
                data_dict[key] = downsample_seg_for_ds_transform2(data_dict[key], self.ds_scales, self.order,
                                                           self.cval, self.axes)
...
def convert_3d_to_2d_generator(data_dict, extra_label_keys=None):
    ...
    if extra_label_keys is not None:
        for key in extra_label_keys:
            ...
            data_dict[key] = data_dict[key].reshape((shp[0], shp[1] * shp[2], shp[3], shp[4]))
...
def convert_2d_to_3d_generator(data_dict, extra_label_keys=None):
    ...
    if extra_label_keys is not None:
        for key in extra_label_keys:
            shp = data_dict['orig_shape_seg']
            current_shape_seg = data_dict['seg'].shape
            data_dict[key] = data_dict[key].reshape((shp[0], shp[1], shp[2], current_shape_seg[-2], current_shape_seg[-1]))
